...
try
{
    ...
    OptionsDescription clientOptions("TableCreator");
    clientOptions.add_options()
        ...
        ("serverSpan",
         ...
         "Server span for the tables.")
        ("tableNames",
         ProgramOptions::value<std::string>(&tableNames),
    ...
    while (std::getline(ss, tableName, ',')) {
      client.createTable(tableName.c_str(), serverSpan);
